...
class App221(Toplevel):
    def __init__(self, master=None):
        ...
        label221 = tk.LabelFrame(self, text="Architecture Information (2,2,1)")
        label221.place(x=50, y=50, width=600, height=400)

        labelPoid = tk.LabelFrame(self, text="Weights for the minimal NMSE")
        labelPoid.place(x=50, y=460, width=400, height=200)

        v=Scrollbar(label221, orient='vertical')
        v.pack(side=RIGHT, fill='y')

        T = Text(label221, width=590, height=490, yscrollcommand=v.set)
        ...
        def affichage():
            for i in range(9):
                fact1 = ("""************ """+"""EPOCH"""+str(i+1)+ """ ************""" + """\n""" +
                """ NMSE of epoch """ + str(i+1) + """: """+ """\"""" + str(a2[i])+ """\"""" + 
                """\n""" 
                + """the weights obtained between the input layer and the hidden layer: """ +
                """\n"""
                )

                affiche_poid_2 = np.zeros((2, 2))
                affiche_poid_3 = np.zeros((1, 2))
                for x in range(2):
                    for y in range(2):
                        affiche_poid_2[x, y] = b2[i, x, y]
                for x in range(1):
                    for y in range(2):
                        affiche_poid_3[x, y] = c2[i, x, y]
        ...
        def affichagePoids():
            fact1 = ("""Minimal NMSE: """ + str(np.min(a2)) + """\n""")
            fact2 = ("""the corresponding weights are: """ + """\n""")
            affiche_2 = np.zeros((2, 2))
            affiche_3 = np.zeros((1, 2))
            for x in range(1):
                for y in range(2):
                    affiche_2[x, y] = b2[np.where(a2 == np.min(a2))[0], x, y]
            for x in range(1):
                for y in range(1):
                    affiche_3[x, y] = c2[np.where(a2 == np.min(a2))[0], x, y]
        ...
        boutonCourbe = tk.Button(self, text="curve", command=courbe_nmse_2)
